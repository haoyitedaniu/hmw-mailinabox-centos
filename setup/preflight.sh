...
if [ -n "$MOUNTED_TMP_AS_NO_EXEC" ]; then
	echo "CentOS-Mail-in-a-Box has to have exec rights on /tmp, please mount /tmp with exec"
	exit
fi

# Check that no .wgetrc exists
if [ -e ~/.wgetrc ]; then
	echo "CentOS-Mail-in-a-Box expects no overrides to wget defaults, ~/.wgetrc exists"
	exit
fi
...
if [ "$ARCHITECTURE" != "x86_64" ]; then
	echo
	echo "WARNING:"
	echo "CentOS-Mail-in-a-Box has only been tested on x86_64 and i686 platform"
